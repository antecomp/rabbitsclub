...
export default {
    ...
    getRecent: (before: number = 2147483647, limit: number = MESSAGE_PAGE_SIZE) => db.select()
    ...
    // Message moderation
    deleteMessage: (messageId: number, deletedBy: number, reason?: string) => db.update(schema.messages)
        .set({
    ...
    addAdminNote: (messageId: number, notedBy: number, note: string) => db.update(schema.messages)
        .set({
            admin_note: note,
